...
void list_add_ascending_by_address(list_t *l, block_t *newblk){
 node_t *newNode = node_alloc(newblk);
    ...
    // If the list is empty or if the new block needs to be inserted at the head
...
void list_add_ascending_by_blocksize(list_t *l, block_t *newblk){
   int newblk_size = newblk->end - newblk->start + 1;
    ...
    // If the list is empty or if the new block needs to be inserted at the head
    if (l->head == NULL || (l->head->blk->end - l->head->blk->start + 1) >= newblk_size) {
        newNode->next = l->head;
        l->head = newNode;
    } else {
        // Traverse the list to find the insert location
...
void list_coalese_nodes(list_t *l){ 
  if (l->head == NULL || l->head->next == NULL) {
        // Either list is empty or has only one block, no coalescing needed
        return;
    }
    ...
    while (current->next != NULL) {
        ...
        if (current->blk->end + 1 == next_node->blk->start) {
            // Merge the current block with the next block
            current->blk->end = next_node->blk->end;

            // Remove next_node from the list
            current->next = next_node->next;
            node_free(next_node);
        } else {
            // Move to the next node if not adjacent
            current = next_node;
        }
    }
}
   ...
   node_t *current = l->head;
    ...
    while (current != NULL) {
        if (current->blk->pid == pid) {
            // Found a block with matching pid
            return true;
        }
        current = current->next;
    }

    // No block with the given pid found
    return false;
}
